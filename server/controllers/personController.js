...
exports.getPeople = async (req, res) => {
    try {
        console.log('Getting people with query:', req.query);
        const { 
            page = 1, 
            limit = 10, 
            sortBy = 'lastName', 
        ...
        console.log(`Found ${people.length} people, total count: ${count}`);
...
exports.createPerson = async (req, res) => {
    ...
    try {
        ...
        if (person) {
            return res.status(400).json({
                success: false,
                error: `A person with email "${email}" already exists. Please use a different email address.`
            });
        }

        // Create new person (company and country are now stored as strings)
        person = new Person({
            firstName,
            lastName,
            ...
            phone,
            position,
            company, // Store company name directly as string
            country, // Store country name directly as string
            city,
            department,
            notes,
            photo,
