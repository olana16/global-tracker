...
exports.getCompany = async (req, res) => {
    try {
        ...
        const people = await Person.find({ company: company.name })
            .select('firstName lastName email position department')
            .sort({ lastName: 1, firstName: 1 });
        
        // Add the people array to the company object
...
exports.createCompany = async (req, res) => {
    ...
    try {
        ...
        // Create new company (country is now stored as string)
        company = new Company({
            name,
...
exports.deleteCompany = async (req, res) => {
    try {
        ...
        await Person.deleteMany({ company: req.params.id });
